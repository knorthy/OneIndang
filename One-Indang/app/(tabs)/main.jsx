...
const handleServicePress = (route) => {
    if (route === 'Services') {
      // ❌ OLD: router.push('/(services)/services'); 
      // ✅ NEW: Navigate to the tab
      router.push('/(tabs)/services'); 
    } else if (route === 'CitizenGuide') {
      // Based on your file structure, citizen.jsx is in the (tabs) folder
      router.push('/(tabs)/citizen');
    } else if (route === 'Students') {
      router.push('/(students)/studmain');
    } else {
      console.log('This route is not linked yet:', route);
    }
  };
        ...
        <View style={styles.servicesGrid}>
          {services.map((service, index) => (
            <TouchableOpacity
              ...
              activeOpacity={0.8}
              // ✅ FIXED: Pass the specific route name to the function
